"""Lucy — Job scraping (Vercel) : France Travail + Adzuna + WTTJ"""
...
def scrape_ft():
    ...
    try:
        tok = ft_token()
        for kw in ["product%20owner%20agile","product%20lead%20AMOA%20MOA","product%20owner%20digital"]:
            url = f"https://api.francetravail.io/partenaire/offresdemploi/v2/offres/search?motsCles={kw}&region=11&typeContrat=CDI&range=0-14&minCreationDate={mn}&maxCreationDate={mx}"
...
def scrape_adzuna():
    jobs, seen = [], set()
    try:
        for q in ["product+owner+agile","product+owner+digital","AMOA+product+owner"]:
            url = f"https://api.adzuna.com/v1/api/jobs/fr/search/1?app_id={AZ_ID}&app_key={AZ_KEY}&results_per_page=15&what={q}&where=Ile-de-France&distance=30&max_days_old=5&sort_by=date&content-type=application/json"
            d = json.loads(fetch(url))
            for j in d.get("results",[]):
                t=j.get("title",""); co=j.get("company",{}).get("display_name","")
                raw=(t+co).lower().replace(" ","")[:24]; jid="az_"+''.join(c for c in raw if c.isalnum())
                if jid in seen: continue
                seen.add(jid)
...
def scrape_wttj():
    jobs, seen = [], set()
    try:
        for kw in ["product owner agile","AMOA product owner","product owner digital"]:
            payload = json.dumps({"requests":[{"indexName":"wttj-jobs-production","params":urllib.parse.urlencode({"query":kw,"filters":"contract_type:CDI AND offices.country_code:FR","hitsPerPage":10,"attributesToRetrieve":"objectID,name,company,published_at,offices,salary_min,salary_max,description,slug"})}]}).encode()
            d = json.loads(fetch(f"https://{WJ_APP.lower()}-dsn.algolia.net/1/indexes/*/queries",{"Content-Type":"application/json",f"X-Algolia-Application-Id":WJ_APP,"X-Algolia-API-Key":WJ_KEY},payload))
            for h in d.get("results",[{}])[0].get("hits",[]):
                ...
                sl=h.get("slug",""); lnk=f"https://www.welcometothejungle.com/fr/companies/{cs}/jobs/{sl}" if cs and sl else "https://www.welcometothejungle.com/fr/jobs"
